...
def validate_file_input(file_path: Path, max_size_mb: int | None = None) -> Path:
    """
    ...
    if resolved_path.stat().st_size == 0:
        msg = f"File is empty: {resolved_path}"
        raise SecurityError(msg)
...
def check_file_size_limit(file_path: Path, max_size_mb: int | None = None) -> None:
    """
    ...
    if file_size_mb > max_size_mb:
        msg = f"File size ({file_size_mb:.1f}MB) exceeds limit ({max_size_mb}MB): {file_path}"
        raise SecurityError(
            msg,
        )
...
def validate_file_type(file_path: Path) -> Literal["pdf", "image"]:
    ...
    msg = f"Unsupported file type: {file_path} (extension: {extension})"
    raise SecurityError(msg)
...
def validate_output_path(output_path: str | None, output_format: str) -> Path:
    ...
    if output_path:
        ...
        if not final_path.parent.exists():
            msg = f"Output directory does not exist: {final_path.parent}"
            raise SecurityError(msg)

        # Check if parent is writable
        if not final_path.parent.is_dir():
            msg = f"Output parent is not a directory: {final_path.parent}"
            raise SecurityError(msg)
